...
export default handleActions(
  {
    ...
    [SET_PLAYER]: (state, {payload}) =>
      update(state, {player: {$set: payload}}),
    [SET_AI]: (state, {payload}) => update(state, {ai: {$set: payload}}),
    [SET_CANDIDATE]: (state, {payload}) =>
      update(state, {candiate: {$set: payload}}),
    [SET_MESSAGE]: (state, {payload}) =>
      update(state, {message: {$set: payload}}),
    [ADD_SWITCH]: state =>
      update(state, {switchCount: {$set: state.switchCount + 1}}),
